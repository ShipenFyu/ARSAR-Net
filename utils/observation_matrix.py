...
from utils.config import p, Nslow, Nfast
...
def get_ob_matrix(batch_size):
    ...
    Phi_fast = torch.matmul(Ah_fast, Aht_fast).expand(batch_size, -1, -1).cuda()
    Phi_slow = torch.matmul(Aht_slow, Ah_slow).expand(batch_size, -1, -1).cuda()

    operator = torch.conj(p['ac']) * torch.conj(p['rc']) * torch.conj(p['sc'])
    operator = operator.to(torch.complex64)

    return Phi_fast, Phi_slow, operator.cuda()
